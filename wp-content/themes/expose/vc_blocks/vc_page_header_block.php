...
function ebor_page_header_shortcode( $atts, $content = null ) {
	...
	if($title && !($subtitle))
		$output .= '<h1 class="main-heading font2 dark"><span style="border-color: '. $colour.' !important; color: '. $colour .' !important;">'. htmlspecialchars_decode($title) .'</span></h1>';
		
	if($title && $subtitle)
		$output .= '<h6 class="dark font3 ebor-font-header" style="border-color: '. $colour.' !important; color: '. $colour .' !important;">'. htmlspecialchars_decode($title) .'</h6>';
	
	if($subtitle)
		$output .= ebor_expose_seperator(2) . '<h2 class="dark font2" style="border-color: '. $colour.' !important; color: '. $colour .' !important;">'. htmlspecialchars_decode($subtitle) .'</h2>';
